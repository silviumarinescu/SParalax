...
(function ($) {
  ...
  $.SParallax = function (SParallaxTransitions, SParallaxTransitionEvents) {
    ...
    this.ApplyParallax = function () {
      ...
      for (var i = 0; i < SParallaxTransitions.length; i++) {
        switch (
          IsTransitionInScreen(SParallaxTransitions[i], scroll, winHeight)
        ) {
          ...
          case 2:
            if (
              $.inArray(
                SParallaxTransitions[i].HtmlObject.selector,
                activeObjects
              ) == -1
            ) {
              DoTransition(
                SParallaxTransitions[i],
                SParallaxTransitions[i].StartParaPoint * winHeight,
                winHeight
              );
            }
            break;
          ...
          case 3:
            if (
              ...
              DoTransition(
                SParallaxTransitions[i],
                SParallaxTransitions[i].EndParaPoint * winHeight,
                winHeight
              );
            }
            break;
    ...
    this.Refresh = function () {
      $("body").height($(window).height() * nrOfScreens);
      this.ApplyParallax();
    };
  ...
  $.SParallax.ScrollTo = function (point, speed) {
    ...
    scroller
      .stop()
      .animate(
        { scrollTop: $(window).height() * point },
        Math.abs(
          Math.round(scroller.scrollTop() / $(window).height()) - point
        ) * sp
      );
  };
  $.SParallax.GetCurrentPoint = function () {
    return Math.round(scroller.scrollTop() / $(window).height());
  };
  //endregion
  //region Classes
  $.SParallax.Transition = function (
    cssProp,
    startVal,
    endVal,
    isPro,
  ...
  function IsTransitionInScreen(trans, scroll, winHeight) {
    if (
      trans.StartParaPoint * winHeight <= scroll &&
      trans.EndParaPoint * winHeight >= scroll
    ) {
      return 1;
    } else {
  ...
  function DoTransition(trans, scroll, winHeight) {
    trans.HtmlObject.css(
      trans.CssProperty,
      (
        (trans.EndValue - trans.StartValue) *
          ((scroll - trans.StartParaPoint * winHeight) /
            (trans.EndParaPoint * winHeight -
              trans.StartParaPoint * winHeight)) +
        trans.StartValue
      ).toString() + trans.IsProcentage
    );
  }
